...
export function gallerySizes(index, sequence=false, layout=null) {
  if (layout) {
    const mobile = sequence ? 'calc(100vw - 40px)' : `calc(${100*layout.mobileSpan/6}vw - ${28*layout.mobileSpan/6+12}px)`;
    const desktop = sequence ? `${76*layout.span/12}vw` : `${(22/3)*layout.span-2}vw`;
    return `(max-width: 700px) ${mobile}, ${desktop}`;
  }
  if (sequence) {
    const nth=index+1;
