...
import { CmsError, text, applyAction } from './model.mjs';
...
export async function uploadPhoto(body) {
  ...
  const showOnHome = body.showOnHome === undefined ? true : body.showOnHome;
  if (typeof showOnHome !== 'boolean') throw new CmsError('Ogiltigt val för förstasidan.');
  if (body.homePosition !== undefined && (!showOnHome || !Number.isInteger(body.homePosition) || body.homePosition < 1 || body.homePosition > homePhotos(state).length + 1)) throw new CmsError('Placeringen på förstasidan är ogiltig.');
  let meta, full, small;
  try {
    ...
    small = await source.clone().rotate().resize({ width: 640, height: 640, fit: 'inside', withoutEnlargement: true }).webp({ quality: 80 }).toBuffer({ resolveWithObject: true });
  } catch { throw new CmsError('Bilden kunde inte läsas. Exportera HEIC/RAW som JPEG och försök igen.'); }
  const id = randomUUID();
  const photo = { id, requestId, width: full.info.width, thumbnailWidth: small.info.width, height: full.info.height, alt: text(body.alt || '', 500), uploadedAt: new Date().toISOString(), status: 'draft', publishedAt: null };
  await Promise.all([writeImage(`${id}-1600`, full.data), writeImage(`${id}-640`, small.data)]);
  const next = await mutateState(current => {
    ...
    const previousOrder = orderedPhotos(current).map(p => p.id);
    photo.showOnHome = showOnHome;
    current.photos.push(photo);
    current.photoOrder = [photo.id, ...previousOrder];
    if (seriesId) current.series.find(s => s.id === seriesId).photoIds.push(id);
    return body.homePosition === undefined ? current : applyAction(current, {type:'photo.update', id:photo.id, homePosition:body.homePosition});
  });
