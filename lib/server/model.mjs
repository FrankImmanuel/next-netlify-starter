...
import { orderedPhotos, homePhotos } from '../photo-order.mjs';
import { validColumnSpan } from '../photo-layout.mjs';
...
export function applyAction(state, action) {
  ...
  switch (action.type) {
    case 'photos.reorder': {
      const ordered = ids(action.ids, next.photos.map(p => p.id));
      if (ordered.length !== next.photos.length) throw new CmsError('Alla bilder måste ingå i ordningen.');
      next.photoOrder = ordered;
      break;
    }
    case 'series.create': {
    ...
    case 'photo.update': {
      ...
      if (action.columnSpan !== undefined) {
        if (action.columnSpan !== null && !validColumnSpan(action.columnSpan)) throw new CmsError('Välj automatisk bredd eller 1–12 kolumner.');
        photo.columnSpan = action.columnSpan;
      }
      if (action.showOnHome !== undefined) {
        if (typeof action.showOnHome !== 'boolean') throw new CmsError('Välj om bilden ska visas på förstasidan.');
        photo.showOnHome = action.showOnHome;
      }
      if (action.homePosition !== undefined) {
        const others = homePhotos(next).filter(p => p.id !== photo.id);
        if (photo.showOnHome === false || !Number.isInteger(action.homePosition) || action.homePosition < 1 || action.homePosition > others.length + 1) throw new CmsError('Placeringen på förstasidan är ogiltig.');
        const order = orderedPhotos(next).filter(p => p.id !== photo.id).map(p => p.id);
        const before = others[action.homePosition - 1];
        order.splice(before ? order.indexOf(before.id) : order.length, 0, photo.id);
        next.photoOrder = order;
      }
...
export function publicCatalog(state) {
  ...
  const photos = orderedPhotos(state).filter(p => p.status === 'published').map(p => ({
    id: p.id, columnSpan: validColumnSpan(p.columnSpan) ? p.columnSpan : null, showOnHome: p.showOnHome !== false, width: p.width, height: p.height, thumbnailWidth: p.thumbnailWidth || null, alt: p.alt, noteTitle: p.noteTitle || '', noteBody: p.noteBody || '', uploadedAt: p.uploadedAt,
