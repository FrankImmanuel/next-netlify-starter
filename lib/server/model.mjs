...
export function publicCatalog(state) {
  ...
  const photos = orderedPhotos(state).filter(p => p.status === 'published').map(p => ({
    id: p.id, showOnHome: p.showOnHome !== false, width: p.width, height: p.height, thumbnailWidth: p.thumbnailWidth || null, alt: p.alt, noteTitle: p.noteTitle || '', noteBody: p.noteBody || '', uploadedAt: p.uploadedAt,
