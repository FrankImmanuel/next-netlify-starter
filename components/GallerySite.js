...
import {photoLayout,galleryLayouts} from '../lib/photo-layout.mjs';
...
export function Gallery({photos,sequence=false,offset=0}) {
 const layouts = sequence ? photos.map(photoLayout) : galleryLayouts(photos,offset);
 photos = photos.map((photo,i)=>({...photo,noteLayout:layouts[i]?'left':(i%8===6 || (i%8===0 && photo.height>photo.width))?'side-right':i%8===1?'vertical':i%8===3?'right':'left',noteTilt:[-2.4,1.6,-.8,-1.5,2.1,-1.2,2.7,-1.8][i%8],noteArrow:(i%8===6 || (i%8===0 && photo.height>photo.width))?'curve':i%4===0?'curve':i%7===3?'curve':null}));
 ...
 return <><div ref={galleryRef} className={sequence?'gallery sequence':'gallery'}>{photos.map((photo,i)=>{const layout=layouts[i];return <figure key={photo.id} className={`work position-${(i+offset)%8}${layout?' custom-span':''}${layout?.solo?' solo-row':''}`} style={layout?{'--photo-row':layout.row,'--photo-span':layout.span,'--photo-start':layout.start,'--mobile-span':layout.mobileSpan,'--mobile-start':layout.mobileStart,'--photo-left':layout.alignment===0?'0':'auto','--photo-right':layout.alignment===2?'0':'auto'}:undefined}><div className="photo-motion"><button className="photo-button" aria-label={`Open photograph ${i+1}: ${photo.alt}`} onClick={e=>{trigger.current=e.currentTarget;setSelected(i);}}><Photo photo={photo} eager={i<2} priority={i===0} sizes={gallerySizes(i+offset,sequence,layout)}/></button><PhotoNote photo={photo} full={sequence}/></div></figure>;})}</div><dialog data-lenis-prevent ref={dialog} className="viewer" aria-label="Photograph viewer" onCancel={e=>{e.preventDefault();close();}} onClick={e=>{if(e.target===dialog.current)close();}} onKeyDown={e=>{if(e.key==='ArrowRight'){e.preventDefault();move(1);}if(e.key==='ArrowLeft'){e.preventDefault();move(-1);}}}>{selected!==null&&<><div className="viewer-top"><span aria-live="polite">{String(selected+1).padStart(2,'0')} / {String(photos.length).padStart(2,'0')}</span><button onClick={close} autoFocus aria-label="Close photograph">Close <X size={22} weight="light" aria-hidden="true"/></button></div><div className="viewer-image"><Photo photo={photos[selected]} eager sizes="(max-width: 700px) calc(100vw - 20px), 90vw" retryable/></div><PhotoNote photo={photos[selected]} full/><div className="viewer-bottom"><button disabled={selected===0} onClick={()=>move(-1)}><ArrowLeft size={18} aria-hidden="true"/> Previous</button>{photos[selected].series.map(series=><Link key={series.slug} href={`/series/${series.slug}`} onClick={close}>{series.title} <ArrowUpRight size={18} aria-hidden="true"/></Link>)}<button disabled={selected===photos.length-1} onClick={()=>move(1)}>Next <ArrowRight size={18} aria-hidden="true"/></button></div></>}</dialog></>;
}
