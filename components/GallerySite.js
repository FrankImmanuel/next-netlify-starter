...
export function Gallery({photos,sequence=false,offset=0}) {
 photos = photos.map((photo,i)=>({...photo,noteLayout:photoLayout(photo)?'left':(i%8===6 || (i%8===0 && photo.height>photo.width))?'side-right':i%8===1?'vertical':i%8===3?'right':'left',noteTilt:[-2.4,1.6,-.8,-1.5,2.1,-1.2,2.7,-1.8][i%8],noteArrow:(i%8===6 || (i%8===0 && photo.height>photo.width))?'curve':i%4===0?'curve':i%7===3?'curve':null}));
