...
home_choices = action('getvalueforkey', WFInput=response, WFDictionaryKey='homeChoices', WFGetDictionaryValueType='Value')
home_labels = action('getvalueforkey', WFInput=home_choices, WFGetDictionaryValueType='All Keys')
home_selected = action('choosefromlist', WFInput=home_labels, WFChooseFromListActionPrompt='Förstasidan: välj placering (även utkast räknas)', WFChooseFromListActionSelectMultiple=False)
home_placement = action('getvalueforkey', WFInput=home_choices, WFDictionaryKey=text(home_selected), WFGetDictionaryValueType='Value')
response = action('downloadurl', WFURL=text(site, '/api/shortcut'), WFHTTPMethod='POST', WFHTTPHeaders=headers, ShowHeaders=True, WFHTTPBodyType='JSON', WFJSONValues=dictionary({'image': encoded, 'seriesId': series, 'homePlacement': home_placement, 'alt': ''}))
check_error(response)
