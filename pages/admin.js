...
export default function Admin() {
  ...
  const [uploadHome, setUploadHome] = useState(true);
  const [uploadPosition, setUploadPosition] = useState(1);
  ...
  const publishUpload = status => run(async () => {
    const { photo } = await request('/api/admin/upload', { image: upload.image, requestId: upload.requestId, seriesId: uploadSeries || null, alt, showOnHome:uploadHome, ...(uploadHome ? {homePosition:Number(uploadPosition)} : {}) });
    const current = await refresh();
    if (status === 'published') {
      const result = await request('/api/admin/catalog', { type: 'photo.update', id: photo.id, status, revision: current.revision });
      setCatalog(result);
    }
    clearUpload(); setNotice(status === 'published' ? 'Bilden är publicerad.' : 'Bilden är sparad som utkast.');
  });
  ...
  return <div className="admin" data-lenis-prevent>
    ...
    {!session ? <p className="admin-loading">Öppnar din studio…</p> : (!session.authenticated || callback) ? <section className="admin-login"><span className="eyebrow">Ditt fotografiska arbete</span><h1>{callback ? 'Välj ditt lösenord' : 'Välkommen tillbaka.'}</h1><p>Logga in för att publicera bilder och arbeta med serier.</p>{session.identityUrl ? <form onSubmit={login}>{!callback && <label>E-post<input name="email" type="email" autoComplete="username" required/></label>}<label>Lösenord<input name="password" type="password" autoComplete={callback ? 'new-password' : 'current-password'} minLength={callback ? 12 : undefined} required/></label><button className="admin-primary" disabled={busy}>{busy ? 'Ett ögonblick…' : callback ? 'Spara lösenord' : 'Logga in'}</button>{!callback && <button type="button" onClick={e => { const email = e.currentTarget.form.elements.email.value; run(async () => { if (!email) throw new Error('Fyll i din e-postadress först.'); const identity = await import('@netlify/identity'); await identity.requestPasswordRecovery(email); setNotice('Kontrollera din e-post för återställningslänken.'); }); }}>Glömt lösenord?</button>}</form> : <p>Netlify-inloggningen behöver aktiveras innan denna studio kan öppnas online.</p>}</section> : catalog && <>
      ...
      {tab === 'photos' && <>
        <section className="upload-panel"><div><h2>Nästa fotografi.</h2><p>Välj din färdigredigerade bild.<br/>Vi gör en webbkopia och behåller originalet orört.</p><input ref={fileRef} id="upload" disabled={busy} className="sr-only" type="file" accept="image/jpeg,image/png,image/webp,image/heic,image/heif" onChange={e => { if (e.target.files[0]) chooseFile(e.target.files[0]); }}/><label className="admin-primary upload-label" htmlFor="upload"><UploadSimple size={19}/>Välj bild</label></div>{upload ? <div className="upload-preview"><img src={upload.preview} alt="Förhandsvisning av vald bild"/><label>Serie<select value={uploadSeries} onChange={e => setUploadSeries(e.target.value)}><option value="">Ingen serie</option>{catalog.series.map(s => <option key={s.id} value={s.id}>{s.title}</option>)}</select></label><HomePlacement enabled={uploadHome} position={uploadPosition} count={homePhotos(catalog).length+1} onChange={(enabled,position)=>{setUploadHome(enabled);setUploadPosition(position);}}/><label>Bildbeskrivning <span>(beskriv motivet, för tillgänglighet)</span><input value={alt} maxLength={500} onChange={e => setAlt(e.target.value)}/></label><div className="admin-actions"><button disabled={busy} className="admin-primary" onClick={() => publishUpload('published')}>{busy ? 'Bearbetar…' : 'Publicera'}</button><button disabled={busy} onClick={() => publishUpload('draft')}>Spara utkast</button><button disabled={busy} onClick={clearUpload}>Avbryt</button></div><p className="admin-hint">Dina val gäller när du publicerar bilden. En serie måste också vara publicerad för att synas.</p><div className="inline-create"><input aria-label="Namn på ny serie" placeholder="Ny serie…" value={newTitle} onChange={e => setNewTitle(e.target.value)} maxLength={120}/><button disabled={busy || !newTitle.trim()} onClick={createSeries}><Plus size={18}/>Skapa</button></div></div> : <div className="upload-placeholder"><Plus size={32} weight="light"/><span>{busy ? 'Förbereder bilden…' : 'JPEG, PNG, WebP eller en bild från Bilder'}</span></div>}</section>
        <div className="admin-section-title"><h2>Bildarkivet</h2><span>Ordna med pilarna eller välj placering i bildens inställningar</span></div>
        {!catalog.photos.length && <p className="admin-empty">Här börjar ditt nya urval. Ladda upp det första fotografiet.</p>}
        <p className="admin-hint">{catalog.photos.filter(p=>p.status==='published'&&!p.alt?.trim()).length} publicerade bilder saknar bildbeskrivning. Öppna en bild för att beskriva motivet.</p><div className="admin-photo-grid">{orderedPhotos(catalog).map((p,i,photos) => <div className="admin-photo-card" key={p.id}><button className="admin-photo" key={p.id} onClick={() => setEditing({ ...p, seriesIds: catalog.series.filter(s => s.photoIds.includes(p.id)).map(s => s.id) })}><img src={thumbnail(p)} alt={p.alt || 'Fotografi'} loading="lazy"/><span>{p.status === 'published' ? 'Publicerad' : 'Utkast'}<span>{new Date(p.uploadedAt).toLocaleDateString('sv-SE')}</span></span></button><div className="admin-photo-order"><span>{p.showOnHome===false?'Ej på förstasidan':`Förstasida · ${homePhotos(catalog).findIndex(x=>x.id===p.id)+1}`}</span><button disabled={busy||i===0} aria-label={`Flytta bild ${i+1} uppåt`} onClick={()=>run(async()=>{await action({type:'photos.reorder',ids:move(photos.map(x=>x.id),i,-1)});setNotice('Bildordningen är sparad.');})}><ArrowUp size={18}/></button><button disabled={busy||i===photos.length-1} aria-label={`Flytta bild ${i+1} nedåt`} onClick={()=>run(async()=>{await action({type:'photos.reorder',ids:move(photos.map(x=>x.id),i,1)});setNotice('Bildordningen är sparad.');})}><ArrowDown size={18}/></button></div></div>)}</div>
      </>}
      ...
      {tab === 'iphone' && <section className="device-panel"><DeviceMobile size={32} weight="light"/><h2>Från Bilder till ditt galleri.</h2><p>Dela en bild till genvägen snabb.studio, välj serie och placering på förstasidan, och öppna förhandsvisningen. Publicera när allt känns rätt.</p><ol><li>Skapa en personlig uppladdningsnyckel nedan.</li><li><a href="/snabb-studio-live.shortcut">Installera den uppdaterade genvägen</a> på din iPhone och klistra in nyckeln.</li><li>Logga in i admin i Safari. Välj sedan en bild i Bilder → Dela → snabb.studio.</li></ol>{catalog.local&&<p className="admin-hint">Den här versionen kör lokalt. iPhone-flödet behöver först en publicerad HTTPS-adress och aktiverad inloggning.</p>}<button className="admin-primary" disabled={busy} onClick={()=>run(async()=>{const result=await request('/api/admin/devices',{});setToken(result.token);await refresh();})}>Skapa uppladdningsnyckel</button>{token&&<div className="token-box"><label>Visas bara nu. Spara nyckeln i din privata genväg.<textarea readOnly value={token} rows={3}/></label><button onClick={()=>run(async()=>{await navigator.clipboard.writeText(token);setNotice('Nyckeln är kopierad.');})}>Kopiera nyckel</button></div>}{catalog.devices.filter(d=>!d.revokedAt).map(d=><div className="device-row" key={d.id}><span>iPhone · {new Date(d.createdAt).toLocaleDateString('sv-SE')}</span><button disabled={busy} onClick={()=>run(async()=>{await request('/api/admin/devices',{id:d.id},'DELETE');setToken('');await refresh();setNotice('Nyckeln är återkallad.');})}>Återkalla</button></div>)}</section>}
      <dialog className="admin-dialog" ref={dialog} onCancel={()=>setEditing(null)}>{editing&&<><header><h2>Fotografi</h2><button aria-label="Stäng bildredigering" onClick={()=>setEditing(null)}><X size={22}/></button></header><img className="edit-photo" src={editing.src||`/api/media/${editing.id}?size=1600`} alt={editing.alt||'Förhandsvisning'}/><label>Bildbeskrivning <span>(beskriv det som syns i bilden)</span><input value={editing.alt} maxLength={500} onChange={e=>setEditing({...editing,alt:e.target.value})}/></label><label>Anteckningsrubrik <span>(valfritt, visas med handstil)</span><input value={editing.noteTitle||''} maxLength={160} onChange={e=>setEditing({...editing,noteTitle:e.target.value})}/></label><label>Anteckning <span>(visas i serie och detaljvy)</span><textarea value={editing.noteBody||''} maxLength={4000} rows={5} onChange={e=>setEditing({...editing,noteBody:e.target.value})}/></label><p className="admin-hint">Rubriken visas med bilden. Anteckningens placering och ritade pilar väljs automatiskt. Spara uppdaterar direkt en redan publicerad bild.</p><label>Bildbredd<select value={editing.columnSpan??''} onChange={e=>setEditing({...editing,columnSpan:e.target.value===''?null:Number(e.target.value)})}><option value="">Automatisk</option>{Array.from({length:12},(_,i)=><option key={i} value={i+1}>{i+1} kolumner{i+1>8?' · Egen rad':''}</option>)}</select></label><p className="admin-hint">12 kolumner på dator. På förstasidan ligger bilder på 1–8 kolumner två per rad. Den andra bilden krymper vid behov men blir aldrig bredare än sitt val. 9–12 får egen rad. Placeringen varieras automatiskt.</p><HomePlacement enabled={editing.showOnHome!==false} position={editing.homePosition ?? Math.max(1,homePhotos(catalog).findIndex(p=>p.id===editing.id)+1)} count={homePhotos(catalog).filter(p=>p.id!==editing.id).length+1} onChange={(enabled,position)=>setEditing({...editing,showOnHome:enabled,homePosition:position})}/><fieldset><legend>Serier</legend>{catalog.series.map(s=><label className="checkbox-row" key={s.id}><input type="checkbox" checked={editing.seriesIds.includes(s.id)} onChange={e=>setEditing({...editing,seriesIds:e.target.checked?[...editing.seriesIds,s.id]:editing.seriesIds.filter(id=>id!==s.id)})}/>{s.title}{s.status==='draft'&&<span>Utkast</span>}</label>)}</fieldset>{error&&<p role="alert" className="error">{error}</p>}<div className="admin-actions"><button disabled={busy} className="admin-primary" onClick={()=>run(async()=>{await action({type:'photo.update',id:editing.id,alt:editing.alt,noteTitle:editing.noteTitle||'',noteBody:editing.noteBody||'',seriesIds:editing.seriesIds,columnSpan:editing.columnSpan??null,showOnHome:editing.showOnHome!==false,...(editing.showOnHome!==false&&editing.homePosition!==undefined?{homePosition:Number(editing.homePosition)}:{})});setEditing(null);setNotice('Bilden är sparad.');})}>Spara</button><button disabled={busy} onClick={()=>run(async()=>{const status=editing.status==='published'?'draft':'published';await action({type:'photo.update',id:editing.id,alt:editing.alt,noteTitle:editing.noteTitle||'',noteBody:editing.noteBody||'',seriesIds:editing.seriesIds,columnSpan:editing.columnSpan??null,showOnHome:editing.showOnHome!==false,...(editing.showOnHome!==false&&editing.homePosition!==undefined?{homePosition:Number(editing.homePosition)}:{}),status});setEditing(null);setNotice(status==='published'?'Bilden är publicerad.':'Bilden är avpublicerad från hela webbplatsen.');})}>{editing.status==='published'?'Avpublicera bild':'Publicera bild'}</button></div><p className="admin-hint">Avpublicering tar bort bilden ur galleriet och serierna direkt. En tidigare publik bildlänk kan fungera i upp till fem minuter. Bilden behålls här och kan publiceras igen.</p></>}</dialog>
    </>}
  </div>;
}

function HomePlacement({enabled,position,count,onChange}) {
  return <fieldset className="home-placement"><legend>Förstasidan</legend><label className="checkbox-row"><input type="checkbox" checked={enabled} onChange={e=>onChange(e.target.checked,position)}/>Visa på förstasidan när bilden är publicerad</label>{enabled&&<label>Placering<select value={position} onChange={e=>onChange(true,Number(e.target.value))}>{Array.from({length:count},(_,i)=><option key={i} value={i+1}>{i===0?'1 · Först':`${i+1}${i===count-1?' · Sist':''}`}</option>)}</select></label>}<p className="admin-hint">Placeringen räknar även utkast som valts för förstasidan. Utkast visas först när de publiceras. Ordningen i serier ändras separat.</p></fieldset>;
}
